...
void list_add_ascending_by_address(list_t *l, block_t *newblk) {
    ...
    // Iterate through the list to find the correct position based on the START address
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) {
    ...
    // Calculate blocksize for the new block
    int newblk_size = newblk->end - newblk->start;

    // Iterate through the list to find the correct position based on blocksize
...
void list_coalese_nodes(list_t *l) {
    ...
    while (current != NULL) {
        ...
        // compare two nodes at a time to see if prev.END + 1 == current.START
        if (prev != NULL && (prev->blk->end + 1) == current->blk->start) {
            // combine them by setting prev.END = current.END
            ...
            // If not adjacent, move to the next nodes
            prev = current;
        }
...
bool list_is_in_by_pid(list_t *l, int pid) {
    ...
    while (current != NULL) {
        // Use the comparePID() function to compare PIDs
        if (comparePID(current->blk->pid, pid) == 0) {
            return true;  // Found a node with matching PID
        }
    ...
    return false;  // PID not found in the list
}
